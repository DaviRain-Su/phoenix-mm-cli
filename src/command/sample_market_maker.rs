...
pub struct SampleMarketMaker {
    ...
    #[structopt(long, default_value = "250.0")]
...
impl SampleMarketMaker {
    pub async fn run(&self) -> anyhow::Result<()> {
        ...
        let trader =
            get_payer_keypair_from_path(&self.keypair_path.clone().unwrap_or(config.keypair_path))?;
        let network_url =
            &get_network(&self.url.clone().unwrap_or(config.json_rpc_url)).to_string();
        ...
        loop {
            match cancel_and_place_quotes(
                ...
                self.quote_size_in_quote_units,
                self.quote_edge_bps,
                self.order_lifetime_in_seconds,
            )
            .await
            {
            ...
            tokio::time::sleep(std::time::Duration::from_millis(
                self.quote_refresh_frequency_in_ms,
            ))
            .await;
        }
    }
